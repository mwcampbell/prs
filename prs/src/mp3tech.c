/*
  ...
  ***************************************************************************
  ...
  This file was modified by Matt Campbell <mattcampbell@pobox.com>
  on November 2, 2002 to be independent of the rest of the mp3info
  package and to represent the playing time of the file as a double
  ...
  if(scantype == SCAN_QUICK) {
	if(get_first_header(mp3,0L)) {
		...
		if(!(scantype == SCAN_FULL)) {
			...
			mp3->seconds = (double)(frame_length(&mp3->header)*mp3->frames)/
